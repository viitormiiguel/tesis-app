...
def crop_landmarks(image, landmarks, landmark_indices):
    ...
    for index in landmark_indices:
        x, y = int(landmarks[index][0] *
                   image_width), int(landmarks[index][1] * image_height)
...
def extrai_feature_hu_moments_imagem_cropped(arqs, crop):
    ...
    colunas = ['hu0_'+crop, 'hu1_'+crop, 'hu2_'+crop,
               'hu3_'+crop, 'hu4_'+crop, 'hu5_'+crop, 'hu6_'+crop]
...
if __name__ == '__main__':
    ...
    path_df = './model/data/cropped_val_face_frontal_4322f_hu_moments.csv'
    ...
    for i in range(len(df)):
        ...
        # .replace('cropped','Openface_Frontal').replace("\\", "\\\\")
        image_path = df['path_image'][i]
        print(image_path)
    ...
    dt_merge.to_csv('./mode/data/cropped_val_face_frontal_and_parts_4322f_hu_moments.csv')